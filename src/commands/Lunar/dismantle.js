...
export const execute = async (instance, message, args) => {
  ...
  if(existing < amount)
    return message.reply(`You only have ${existing} which isnt ${amount} :(`)

  await instance.db.pool.query("DELETE FROM CLAIMS WHERE id IN (SELECT id FROM CLAIMS WHERE user_id=$1 AND animal=$2 AND color=$3 ORDER by id LIMIT $4)", [message.author.id, animal, color, amount])
  const toAdd = amount * 5;
  addBalance(instance, message.author, message.guild, toAdd, `Destroyed ${amount} of ${color} ${animal}`)

  return message.reply(`You gained ${toAdd} currency for destroying ${amount} ${capitalise(color)} ${capitalise(animal)}, begone!`)
